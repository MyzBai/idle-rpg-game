...
/**@typedef {import('../damageCalc.js').ConversionTable} ConversionTable */
/**@typedef {import('../modDB.js').RawStatMod} Stat */
/**@typedef {import('../modDB.js').StatModifier} StatModifier */

/**
 * @typedef SkillSettings
 * @property {number} numSupports
 */
...
function setActiveAttackSkill(name) {
    ...
    if (!data) {
        window.alert(`could not find attack skill with name ${name}. please contact the developer if you suspect this to be a bug`);
        return;
    }
...
function viewSkill(name, type) {
    ...
    /**@type {AttackSkillJson} skill */
    let skill = type === 'attack' ? attackSkillsCollection.find(x => x.name === name) : supportsCollection.find(x => x.name === name);
    if (!skill) {
...
export class AttackSkill {
    /**@param {AttackSkillJson} skillJson */
    constructor(skillJson) {
        ...
        const rebuildModList = () => {
            ...
            for (const support of supports.filter(x => x)) {
                const statMods = convertStatMods(support.mods.flatMap(x => x.stats), support);
...
function save(obj) {
    ...
    let supportNames = attackSkill.getSupports().filter(x => x).map(x => x.name);
    obj.skills = {
        attackSkill: {
